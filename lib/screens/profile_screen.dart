                  ...
                  FutureBuilder<DocumentSnapshot>(
                    ...
                    builder: (context, userSnapshot) {
                      ...
                      if (userSnapshot.hasError) {
                        return Center(child: Text('Erreur de chargement du profil : ${userSnapshot.error}'));
                      }

                      final userData = userSnapshot.data?.data() as Map<String, dynamic>?;

                      // Formatage de la date d'inscription avec intl et gestion du champ createdAt
                      String formattedDate = 'Inconnu';
                      if (userData != null && userData['joinDate'] != null) {
                        final Timestamp timestamp = userData['joinDate'];
                        final date = timestamp.toDate();
                        formattedDate = DateFormat("dd MMMM yyyy", "fr_FR").format(date);
                      }
                      ...
                      return StreamBuilder<QuerySnapshot>(
                        ...
                        builder: (context, itemsSnapshot) {
                          ...
                          if (itemsSnapshot.hasError) {
                            return Center(child: Text('Erreur de chargement des statistiques : ${itemsSnapshot.error}'));
                          }
                          ...
                          final found = items.where((doc) {
                            final data = doc.data() as Map<String, dynamic>?;
                            final status = data?['status'] ?? '';
                            return status.toString().toLowerCase() == 'trouvé';
                          }).length.toString();
                          ...
                          return StreamBuilder<QuerySnapshot>(
                            ...
                            builder: (context, helpedSnapshot) {
                              ...
                              if (helpedSnapshot.hasError) {
                                return Center(child: Text('Erreur de chargement des aides : ${helpedSnapshot.error}'));
                              }
                              ...
                              return _buildUserCard(
                                ...
                                email: userData?['email'] ?? user.email ?? 'Aucun email',
...
//                           final date = (userData!['createdAt']).toDate();
//                           formattedDate = DateFormat("dd MMMM yyyy", "fr_FR").format(date);
//                         }

//                         // Valeurs fictives pour les stats (à remplacer par Firestore si besoin)
//                         String reported = "0";
//                         String found = "0";
//                         String helped = "0";
//                       return StreamBuilder<QuerySnapshot>(
...
//                           // Calcul du nombre total d'items signalés
//                           final reported = items.length.toString();

//                           // Calcul du nombre d'items retrouvés en vérifiant la présence et la valeur du champ 'status'
//                           final found = items.where((doc) {
//                             final data = doc.data() as Map<String, dynamic>?; // Cast sécurisé
//                             final status = data?['status'] ?? '';
//                             return status.toString().toLowerCase() == 'trouvé';
//                           }).length.toString(); 
...
//                                             return joinDate.toDate().toString().split(' ')[0];
//                                           }
//                                           return 'Inconnu';
//                                         }(), 
//                                 photoUrl: userData?['photoUrl'] ?? user.photoURL,
